...
class LearningGoalBase(BaseModel):
    ...
    goal_type: str = Field(default='knowledge', regex='^(knowledge|skill|understanding|attitude)$')
...
class UnitConceptCreate(BaseModel):
    ...
    concept_id: UUID
    emphasis_level: str = Field(default='supporting', regex='^(central|major|supporting)$')
...
class UnitOfInquiryUpdate(BaseModel):
    ...
    end_date: Optional[date] = None
    unit_status: Optional[str] = Field(None, regex='^(planning|active|completed|archived)$')
